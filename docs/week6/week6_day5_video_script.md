...
### Кадр 2 — Веб-чат в браузере (~15 с) ★

Открыть в Chrome: `http://localhost:7788/`

Появится тёмная страница «💬 Day 30 · Local LLM», в правом верхнем углу — статус `✓ qwen2.5:7b · index: 47 chunks`, переключатель RAG/Chat, поле API-ключа. Ввести `demo-2026` в поле ключа.

(Быстрый параллельный кадр — `http://localhost:7788/health` возвращает тот же JSON, что и раньше — можно упомянуть одной секундой.)
...
### Кадр 5 — /v1/rag через веб-чат (~40 с) ★ КЛЮЧЕВОЙ

В браузере, на странице `http://localhost:7788/`:
1. Убедиться, что переключатель на **RAG (по докам)** и ключ введён.
2. Ввести: `Что такое RAG в двух предложениях?`
3. Enter.

Внизу появится сообщение пользователя. Через 1-2 сек — ответ бота с:
- **Текстом ответа с маркерами [S1]/[S2]/[S3]**.
- Блоком «Источники:» под ответом — список `[S1] day26_research.md (sim=0.612)` × 3.
- Метриками: `wall: 1080 ms · eval: 760 ms · tokens: 38`.

Пауза 5 сек — видно и красивый ответ, и sources, и метрики. **Это главный кадр.**
...
### Кадр 9 — Доступ по сети (телефон) (~30 с) ★
...
**Взять телефон**, открыть в Chrome: `http://192.168.1.42:7788/`.

На экране телефона тот же тёмный чат-UI. Ввести ключ, переключить на RAG, задать любой вопрос — модель отвечает, sources показывает. **«Доступ по сети» задания закрыт, причём с полноценным UI, а не с голым JSON.**
